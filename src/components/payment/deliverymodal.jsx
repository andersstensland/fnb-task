    ...
    <AlertDialog isOpen={isOpen} onDismiss={() => setIsOpen(false)}>
      ...
      <AlertDialogContent className="rounded-t-3xl p-6 w-full max-w-md mx-auto">
        ...
        <div className="space-y-4 mt-4">
          <div className={`flex items-center justify-between space-x-4 p-4 rounded-lg bg-white shadow-md cursor-pointer`} onClick={() => handleOptionClick('A')}>
            <label htmlFor="option-A" className="text-base font-semibold flex-grow-1">
              Pick up point A
            </label>
            <input type="radio" id="option-A" />
          </div>
          <div className={`flex items-center justify-between space-x-4 p-4 rounded-lg bg-white shadow-md cursor-pointer`} onClick={() => handleOptionClick('B')}>
            <label htmlFor="option-B" className="text-base font-semibold flex-grow-1">
              Pick up point B
            </label>
            <input type="radio" id="option-B" />
          </div>
          <div className={`flex items-center justify-between space-x-4 p-4 rounded-lg bg-white shadow-md cursor-pointer`} onClick={() => handleOptionClick('C')}>
